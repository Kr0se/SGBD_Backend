...
public class FitxerBDService {
    ...
    public FitxerBD getVideo(String id) throws IllegalStateException, IOException {
        ...
        //video.setFitxerUsuariList(file.getMetadata().get("fitxerUsuariList").);
        return fitxerBD;
    }
    ...
    /*public void addFileTextUser(FitxerUsuari fu, String id) throws IOException {
        GridFSFile file = gridFsTemplate.findOne(new Query(Criteria.where("_id").is(id)));

        byte[] l = file.getMetadata().get("fitxerUsuariList").toString().getBytes(StandardCharsets.UTF_8);

         List<byte[]> f = Arrays.asList(l);
